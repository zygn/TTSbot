...
async def cmd_tts(ctx, *, msg):
    ...
    if not (set_voice and set_lang):
        ...
        res, obj = await tts_resolver(set_voice, set_lang, msg)
        if res is False:
            await ctx.send(obj, delete_after=15)
            return
        else:
            await voice_send(ctx, obj)


@bot.command(aliases=['set'])
async def cmd_set(ctx, *, msg):
    uid = str(ctx.author.id)
    _chars = msg.split(' ')
